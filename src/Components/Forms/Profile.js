...
const Profile = () => {
  ...
  return (
    <main>
      ...
      {profile.name ? (
        <div className=" my-4 w-full overflow-auto rounded-md border-2 border-slate-900 light:border-slate-500 light:p-[1px] md:w-1/2 xl:w-1/3">
          {userType === "teacher" ? (
            <PiUser className="mx-auto my-6 rounded-full border-2 border-slate-900 p-1 text-9xl light:border-slate-300 md:p-2" />
          ) : (
            <PiStudent className="mx-auto my-6 rounded-full border-2 border-slate-900 p-1 text-9xl font-light light:border-slate-300 md:p-2" />
          )}
